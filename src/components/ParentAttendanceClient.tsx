...
const ParentAttendanceClient = ({ parent, childrenData }: ParentAttendanceClientProps) => {
  ...
  // Calculate attendance statistics for current child
  const totalDays = currentChild.attendances.length;
  const presentDays = currentChild.attendances.filter(record => record.present).length;
  const absentDays = totalDays - presentDays;
  const attendanceRate = totalDays > 0 ? Math.round((presentDays / totalDays) * 100) : 0;

  // Group attendance by class
  const attendanceByClass = currentChild.attendances.reduce((acc, record) => {
  ...
  const getFilteredAttendance = () => {
    ...
    switch (selectedPeriod) {
      ...
      case 'term':
        ...
        break;
      default:
        return currentChild.attendances;
    }

    return currentChild.attendances.filter(record => new Date(record.date) >= startDate);
  };

  // Prepare calendar events
  const calendarEvents = currentChild.attendances.map(record => ({
    id: record.id,
          ...
          <div className="border-b border-gray-200">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">
                {currentChild.name} {currentChild.surname} - {currentChild.class?.name || 'No Current Class'}
              </h2>
            </div>
...
export default ParentAttendanceClient;